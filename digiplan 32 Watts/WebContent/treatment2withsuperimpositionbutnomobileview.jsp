...
<canvas ng-mouseover="stoptimer();" ng-mouseleave="callintervalonplay();" id="layer1"
style="z-index: 1;
position:relative; opacity:1;  " width="400px" height="400px"  >
This text is displayed if your browser does not support HTML5 Canvas.
</canvas>  

<canvas align="center" id="layer1_1" style="z-index: 1; opacity:0.7; 
position:absolute ;
left:0;
top:20px;" width="400px" height="400px"  >
This text is displayed if your browser does not support HTML5 Canvas.
</canvas>
...
This text is displayed if your browser does not support HTML5 Canvas.
 ...
 <div ng-show="urlarray.length>0" class="player row" style="background-color:black; margin-top:-15px; padding: 10px; ">
    ...
    <input style=" margin-top: -5px; margin-left:-15px; width: 100%" type="range" class="slider" ng-change="slidervaluechanged(slidervalue); showcommenttextarea=true;"  ng-model="slidervalue"   min="0" max="{{urlarray.length-1}}" >
      ...
      */
      ...
      $scope.showfrontimage=function(){
        ...
        $scope.callinterval=function(){
            intervalvar=$interval($scope.replacebytime, 20000);
            }
        ...
        $scope.show3dview=function(){
        	...
        	"	position:relative;\" width=\"400px\" height=\"400px\" >"+
        	  ...
        	  */ canvas = document.getElementById('layer1');
        	  canvas2 = document.getElementById('layer1_1');
             viewer = new JSC3D.Viewer(canvas,{"otherCanvas":canvas2});
             ...
             numOfLoaded = 0;
               ...
               //	viewer.generateBackground();
                ...
                canvas2 = document.getElementById('layer1_1');
                /* c=canvas2.getContext("3d");
                c.globalAlpha=0.5;
                */ viewer2 = new JSC3D.Viewer(canvas2);
                theScene2 = new JSC3D.Scene;
                numOfLoaded2 = 0;
               	
                   
                   viewer2.setParameter('BackgroundColor1', '#00FFFFFF');
                   viewer2.setParameter('BackgroundColor2', '#00FFFFFF');
                   viewer2.setParameter('Renderer', 'webgl');
                  	viewer2.setRenderMode('texturesmooth');
                  	//viewer.setDefinition('high');
                  //	viewer.generateBackground();
               	viewer2.setParameter('Definition', 'high');
               	viewer2.setParameter("Background", 'Off'); 
                   viewer2.init();
                   viewer2.update();
            ...
            $http.post("rest/stlfiles", JSON.stringify({"user":$scope.patientid,"category":$scope.category})).then(function(value) {
            	...
            	viewer2.setParameter("SceneUrl",$scope.urlarray[1]);
            	viewer2.setParameter('ModelColor', '#888888');
            	viewer2.init();
